...
if(request.getParameter("listar").equals("listar")){
   ...
   try{
        ...
        rs=st.executeQuery("select h.*,d.iddoctor,d.doc_nombre from horarios h inner join doctores d on h.iddoctor = d.iddoctor;");
    while(rs.next())
{%>
<tr>
    ...
    <td><i title="modificar" class="bi bi-pencil-square" onclick="rellenaredit('<%out.print(rs.getString(1));%>','<%out.print(rs.getString(2));%>','<%out.print(rs.getString(3));%>','<%out.print(rs.getString(4));%>')"></i><i title="eliminar" class="bi bi-trash" onclick="$('#idhorario_e').val('<%out.print(rs.getString(1));%>')" data-bs-toggle="modal" data-bs-target="#exampleModal"></i></td>
</tr>
...
try {
            ...
            ResultSet rs = st.executeQuery("select count(*) FROM horarios where iddoctor='" + doctor + "'");
            rs.next();
...
out.print("<div class='alert alert-danger' role='alert'>campo vacio nombre</div>");
 ...
 try{
        Statement st=null;
        st=conn.createStatement();
        st.executeUpdate("update  horarios set hora_atencion='"+atencion+"',hora_estado='"+estado+"',iddoctor='"+doctor+"' where idespecialidad='"+pk+"'");

...
<tr>
    ...
    <td><i title="modificar" class="fas fa-edit" onclick="rellenaredit('<%out.print(rs.getString(1));%>','<%out.print(rs.getString(2));%>','<%out.print(rs.getString(3));%>','<%out.print(rs.getString(4));%>','<%out.print(rs.getString(5));%>','<%out.print(rs.getString(6));%>')"></i><i title="eliminar" class="fas fa-trash" onclick="$('#idusuario_e').val('<%out.print(rs.getString(1));%>')" data-toggle="modal" data-target="#exampleModal"></i><a class="fas fa-eye" href="vista_2.jsp?coco=<% out.print(rs.getString(1));%>"></a></td>
</tr>
...
String foto =request.getParameter("usu_profile");
...
try {
        ...
        st.executeUpdate("INSERT INTO usuario(usu_nombre, usu_clave,usu_estado,usu_tipo,idpersonal,usu_profile) VALUES ('"+nombre+"',md5('"+clave+"'),'"+estado+"','"+tipo+"','"+persona+"')");
...
out.print("<div class='alert alert-success' role='alert'>valor vacio</div>");
 ...
 try{
        Statement st=null;
        st=conn.createStatement();
        st.executeUpdate("update  usuario set usu_nombre='"+nombre+"', usu_clave=md5('"+clave+"'),usu_estado='"+estado+"', usu_tipo='"+tipo+"',idpersonal='"+persona+"' where idusuario='"+pk+"'");
...
else if(request.getParameter("listar").equals("contar")){
try{
        Statement st=null;
        ResultSet rs=null;
        st=conn.createStatement();

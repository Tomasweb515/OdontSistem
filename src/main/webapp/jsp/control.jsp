...
<%
    ...
    String codigo = request.getParameter("id");
    int prueba = Integer.parseInt(codigo); // Convirtiendo el código de String a Integer

    // Convertimos el valor total a un número entero
    int total;
    try {
        total = Integer.parseInt(request.getParameter("total")); // Conversión segura
    } catch (NumberFormatException e) {
        out.println("Error al convertir el total: " + e.getMessage());
        return; // Salir del proceso si hay un error en la conversión
    }
    ...
    try {
        ...
        try (PreparedStatement pst = conn.prepareStatement(
            ...
            try (ResultSet pk = pst.executeQuery()) {
                if (pk.next()) {
                    // Si la cita existe y está pendiente, actualizamos su estado y total
                    try (PreparedStatement updateStmt = conn.prepareStatement(
                            "UPDATE citas SET ci_estado = 'Finalizado', total = ? WHERE idcita = ?")) {
                        updateStmt.setInt(1, total); // Aquí se usa el valor entero correctamente
                        updateStmt.setInt(2, prueba);
                        updateStmt.executeUpdate();
                        out.println("<div class='alert alert-success' role='alert'>Datos modificados con éxito!</div>");
                    }
                } else {
                    out.println("<div class='alert alert-danger' role='alert'>La cita no existe o no está pendiente.</div>");
                }
            }
        }
    } catch (Exception e) {
        out.println("Error en la operación SQL: " + e);
    }
}
